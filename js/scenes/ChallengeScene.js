...
import {
  ...
  drawMist, drawStars, drawSparkle, drawTree, getFont,
} from '../utils/color.js';
...
export class ChallengeScene {
  ...
  _startReward() {
    ...
    var done = 0;
    for (var i = 0; i < XINFA_LIST.length; i++) { if (this._done[i]) done++; }
    this._stars = done >= XINFA_LIST.length ? 3 : done >= 4 ? 2 : 1;
  ...
  render(ctx) {
    ...
    ctx.fillText('完成 ' + done + ' / ' + XINFA_LIST.length, w / 2, 40);

    for (var j = 0; j < XINFA_LIST.length; j++) {
      var y = 62 + j * 54;
      var rh = 46;
    ...
    if (this._showReward) {
      ...
      if (this._rewardPhase >= 1) {
        this._drawRewardCell(ctx, rx + 16, cellY, cellW, 34, '修为', '+' + (5 + this._stars * 3), C.red);
      }
      if (this._rewardPhase >= 2) {
        this._drawRewardCell(ctx, rx + 16 + cellW + gap2, cellY, cellW, 34, '连击', this.player.streak + '天', C.gold);
      }
      if (this._rewardPhase >= 3 && this._stars) {
        this._drawRewardCell(ctx, rx + 16, cellY + 40, rw - 32, 34, '星级评定', this._stars + '星', C.goldLight);
      }
    if (this._rewardPhase >= 4 && this._levelUp) {
      if (!this._levelUpPlayed && this.sm.audio) {
        this.sm.audio.playLevelUp();
        this._levelUpPlayed = true;
      }
      this._drawRewardCell(ctx, rx + 16, cellY + 80, rw - 32, 34, '等级突破', 'Lv.' + this.player.level, C.red);
    }
    ...
    if (this._toastT > 0) {
      ...
      ctx.fillStyle = 'rgba(60,45,30,0.82)';
      ctx.fill();
      ctx.fillStyle = '#fff';
      ctx.font = getFont(w, 12, 'sans');
      ctx.textAlign = 'center';
  ...
  _drawRewardCell(ctx, x, y, w, h, label, val, color) {
    ...
    ctx.stroke();
    ctx.fillStyle = C.inkLight;
    var cfs = Math.min(fs(ctx.canvas.width, 11), 14);
    ctx.font = cfs + 'px "PingFang SC", "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
